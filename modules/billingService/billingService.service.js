...
async function createBillingServiceService(
  service_name,
  description,
  default_price,
  category,
  ...
  const existingServiceName = await BillingService.findOne({
    where: { service_name: trimServiceName, is_deleted: false },
  });
...
async function updateServiceService(service_id, updateField) {
  ...
  const allowedFields = ['service_name', 'description', 'default_price', 'category'];
  ...
  for (const field of allowedFields) {
    ...
    if (typeof value === 'string') {
      ...
      if (isNaN(value) || value < 0)
        return { success: false, message: 'Input must be a valid non negative number.' };
      trimmed = value;
    } else {
      continue;
    }
