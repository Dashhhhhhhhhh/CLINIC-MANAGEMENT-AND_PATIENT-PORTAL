...
async function createRoleService(role_id, role_name, description, active) {
  ...
  if (!sNameRegex.test(cleanedRoleName)) {
    return {
      success: false,
      message: "Role name can only contain letters, spaces, hyphens (-), and apostrophes (').",
    };
  }
...
async function updateRoleService(role_id, updateField) {
  ...
  const update = ['role_name', 'description'];
...
async function toggleRoleStatusService(role_id, active) {
  ...
  if (role.active === active) {
    return {
      success: false,
      message: active ? 'Role is already active' : 'Role is already inactive',
    };
  }
  ...
  return {
    success: true,
    message: role.active ? 'role activated' : 'role deactivated',
    role: role.get({ plain: true }),
  };
}
