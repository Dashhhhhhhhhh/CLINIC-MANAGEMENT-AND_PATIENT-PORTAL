...
const {
  ...
  updatePatient,
  togglePatientStatus,
} = require('../patients/patients.model');
...
async function registerPatientService(
  ...
  const existingPatient = await Patient.findOne({
    where: { first_name: update.first_name, last_name: update.last_name, birthdate: parsedDate },
  });
...
async function togglePatientStatusService(patient_id, active) {
  ...
  if (patient.active === active) {
    return {
      success: false,
      message: active ? 'Patient is already active' : 'Patient is already inactive',
    };
  }
