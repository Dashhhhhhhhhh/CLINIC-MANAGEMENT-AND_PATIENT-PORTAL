const { createResultService, getAllResultService, getResultByIdService, updateResultService, toggleResultDeleteService } = require("./result.service");
...
async function createResultController (req, res) {
    ...
    // Get staff_id from authenticated staff (same logic as Billing)
...
async function getResultByIdController(req, res) {
  try {
    ...
    if (!result.success) return res.status(404).json({ success: false, message: "Result not found. "});
