...
async function createBillingItemService(
  billing_id,
  service_id,
  description,
  quantity,
  ...
  await Billing.update({ total_amount: roundTotal }, { where: { billing_id: cleanBillingId } });
...
async function updateBillingItemService(billing_item_id, updateField) {
  ...
  const allowedFields = ['service_id', 'description', 'quantity', 'unit_price'];
...
async function toggleDeletebillingItemService(billing_item_id, billing_id, is_deleted, toggled_by) {
  ...
  if (existingItem.billing_id !== billing_id) {
    return {
      success: false,
      message: 'This billing item does not belong to the provided billing.',
    };
  }
  ...
  if (existingItem.is_deleted === is_deleted) {
    return {
      success: false,
      message: is_deleted ? 'Billing item is already deleted.' : 'Billing item is already active.',
    };
  }
  ...
  await Billing.update({ total_amount: safeTotal }, { where: { billing_id } });
