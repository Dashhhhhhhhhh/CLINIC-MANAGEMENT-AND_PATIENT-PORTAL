...
async function registerStaffService(
  user_id,
  first_name,
  middle_initial,
  last_name,
  ...
  if (!user_id || !first_name || !last_name || !position_id) {
    return {
      success: false,
      message: 'Please provide all required fields (user_id, first_name, last_name, position_id).',
    };
  }
  ...
  const countToday = await Staff.count({
    where: Sequelize.where(Sequelize.fn('DATE', Sequelize.col('created_at')), localToday),
  });
...
async function toggleStaffStatusService(staff_id, active) {
  ...
  if (staff.active === active) {
    return {
      success: false,
      message: active ? 'staff is already active.' : 'staff is already inactive',
    };
  }
  ...
  return {
    success: true,
    message: staff.active ? 'Staff activated successfully.' : 'Staff deactivated successfully.',
    staff: staff.get({ plain: true }),
  };
}
