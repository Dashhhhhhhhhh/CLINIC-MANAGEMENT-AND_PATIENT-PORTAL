...
function Doctors() {
  ...
  useEffect(() => {
    const fetchAvailableSpecialization = async () => {
      try {
        const response = await fetch('http://localhost:3000/specializations/available');
        const data = await response.json();
        ...
        <tbody>
          {doctor.map((doctor, index) => (
            <tr key={doctor.doctor_id}>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>{doctor.doctor_id}</td>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>{doctor.first_name}</td>
            </tr>
          ))}
        </tbody>
      </table>
        ...
        <form>
          ...
          >
            ...
            {availableSpecialization.map(specialization => (
              ...
              >
                {specialization.specialization_name} ({specialization.description})
              </option>
            ))}
          </select>
